...
int manhattanDistance(ii first, ii second) {
...
int main() {
    ...
    fringe.push(iii(manhattanDistance(ii(pacR, pacC), ii(foodR, foodC)), ii(pacR, pacC)));
    ...
    while (!fringe.empty()) {
        ...
        ii pos = state.second;
        if (state.first > dist[pos.first][pos.second] + manhattanDistance(pos, ii(foodR, foodC))) continue;
        if (pos.first == foodR && pos.second == foodC) {
            break;
        }
        rep(d,0,4) {
            ii next = ii(pos.first + dr[d], pos.second + dc[d]);
            int priority = dist[pos.first][pos.second] + 1 + manhattanDistance(next, ii(foodR, foodC));

            // Only compare the distance from the start...
            if (inbounds(next) && 
                (dist[next.first][next.second] > dist[pos.first][pos.second] + 1 || dist[next.first][next.second] == -1) &&
